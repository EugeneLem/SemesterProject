...
for i = 2:size(Z,1)
    obj = obj + L2NW_mean(Z(i,:), Z(1:i-1,:), Y_1(1:i-1), [sig_l(1),sig_f(1), lambda], 3)^2;   
end
opti.minimize(obj);
...
for i = 2:size(Z,1)
    obj = obj + L2NW_mean(Z(i,:), Z(1:i-1,:), Y_2(1:i-1), [sig_l(2),sig_f(2), lambda], 3)^2;   
end
opti.minimize(obj);
...
for i = 2:size(Z,1)
    obj = obj + L2NW_mean(Z(i,:), Z(1:i-1,:), Y_3(1:i-1), [sig_l(3),sig_f(3), lambda], 3)^2;   
end
opti.minimize(obj);
...
for i = 2:size(Z,1)
    obj = obj + L2NW_mean(Z(i,:), Z(1:i-1,:), Y_4(1:i-1), [sig_l(4),sig_f(4), lambda], 3)^2;   
end
opti.minimize(obj);
